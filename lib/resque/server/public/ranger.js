$(function() {
  ...
  var relatizer = function(){
    ...
    if ($(this).parents("a").length > 0 || $(this).is("a")) {
      $(this).relatizeDate();
      if (!$(this).attr('title')) { $(this).attr('title', dt); }
    } else {
      var dt_html        = '<span class="date_time">' + dt + '</span>';
      var relatized_html = '<span class="relatized_time">' + relatized + '</span>';
      $(this).html('<a href="#" class="toggle_format" title="' + dt + '">' + dt_html + relatized_html + '</a>');
    }
  };

module Resque
  module Plugin
    ...
    # For grater speed of execution make sure that the object (job) that is
    # given responds to the hooks method.
    def get_hook_names(job, hook_method_prefix)
      methods = (job.respond_to?(:hooks) && job.hooks) || job_methods(job)
      methods.select{|m| m.starts_with?(hook_method_prefix)}.sort
    end
